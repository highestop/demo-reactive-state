...
import { createServerState, IServerState, IServerStateService } from "./server-state"
...
interface IStoreBase<D extends IStoreStateData> {
    ...
    setState: <K extends IStoreStateKey<D>>(key: K, state: ReturnType<D[K]['getState']>) => void
...
export class StateStore<D extends IStoreStateData> implements IStateStore<D> {
    ...
    public setState = <K extends IStoreStateKey<D>>(key: K, state: ReturnType<D[K]['getState']>) => {
        if (!this.stateStore) {
            throw Error('store has not been inited')
        }
    ...
    public subscribeStateChange = <K extends keyof D>(key: K, callback: (state: ReturnType<D[K]['getState']>) => any): IStateSubscription => {
        if (!this.stateStore) {
            throw Error('store has not been inited')
        }
