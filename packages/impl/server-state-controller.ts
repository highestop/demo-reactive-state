...
function createServerState<T extends ServerStateToken>(
    token: T
): IServerStateController<ServerStateTokenTypeMap[T], ServerStateTokenTypeMap[T] | undefined>
function createServerState<T extends ServerStateToken>(
    token: T,
...
>(controllers: D, service: IServerStateService) {
    Object.values(controllers).forEach((controller) => controller?.connect(service))
}
